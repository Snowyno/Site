            ...
            <div class="lg:flex lg:items-center lg:justify-between">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <h2 class="text-lg font-bold leading-7 text-zinc-700 dark:text-gray-50 sm:truncate sm:text-xl sm:tracking-tight">
                        @lang('messages.t_welcome_back') <?php echo ", ".auth()->user()->username; ?>!
                    </h2>
                    ...
                    <div class="mt-1 flex flex-col sm:mt-0 sm:flex-row sm:flex-wrap sm:space-x-6 rtl:space-x-reverse">
                        ...
                        <div class="mt-2 flex items-center text-sm text-gray-500 dark:text-zinc-200">
							...
							@lang('messages.t_member_since')  <?php echo date('d/m/Y', strtotime(auth()->user()->created_at)); ?> 
                        </div>
				...
				<div class="rounded-lg border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 shadow-sm px-4 py-4 sm:px-5">
					...
					<div class="space-y-4">
						@forelse ($latest_messages as $message)
							...
							<a href="{{ url('messages', $message['uid']) }}" class="flex cursor-pointer items-center justify-between space-x-2 rtl:space-x-reverse">
								...
								<tr class="intro-x shadow-sm bg-white dark:bg-zinc-800 rounded-md h-16" wire:key="freelancer-dashboard-latest-order-{{ $order->uid }}">
									...
									<td class="px-5 py-3 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md w-72 rtl:text-right">
										<div class="flex items-center space-x-3 rtl:space-x-reverse">
											...
											<div>
												...
												{{-- Title --}}
												<span class="font-medium whitespace-nowrap truncate block max-w-sm hover:text-primary-600 dark:text-white text-sm" title="{{ $order->gig->title }}">
													{{ $order->gig->title }}
												</span>
									...
									<td class="px-5 py-3 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md text-center">
										@if ($order->expected_delivery_date)
											<span class="text-sm whitespace-nowrap font-normal text-zinc-600 dark:text-zinc-300"><?php echo date("d/m/Y \à\s h:i:s A", strtotime($order->expected_delivery_date)); ?> <!--{{ format_date($order->expected_delivery_date, config('carbon-formats.F_j,_Y_h_:_i_A')) }}--></span>
										@elseif (in_array($order->status, ['pending', 'proceeded']) && !$order->is_requirements_sent)
											...
											<span class="text-sm font-medium text-gray-700 dark:text-gray-400">
												{{ __('messages.t_agir_ate') }}: <span id="TEMPORESTANTE<?php echo $order->id;?>"></span>
												<script>
