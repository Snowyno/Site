                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            border-gray-300 dark:border-zinc-500" id="statusFilterSelect"  name="statusFilterSelect" onchange="changeFilter();">
                                                <option class="dark:bg-zinc-700" value="todos" <?php echo (!isset($_GET['statusFilter'])) ? "selected" : ""; ?>> @lang('messages.t_all') </option>
                                                <option class="dark:bg-zinc-700" value="pending" <?php echo (isset($_GET['statusFilter']) AND $_GET['statusFilter'] == "pending") ? "selected" : ""; ?>> @lang('messages.t_pending') </option>
                                                <option class="dark:bg-zinc-700" value="proceeded" <?php echo (isset($_GET['statusFilter']) AND $_GET['statusFilter'] == "proceeded") ? "selected" : ""; ?>> @lang('messages.t_proceed') </option>
                                                <option class="dark:bg-zinc-700" value="delivered" <?php echo (isset($_GET['statusFilter']) AND $_GET['statusFilter'] == "delivered") ? "selected" : ""; ?>> @lang('messages.t_delivered') </option>
                                                <option class="dark:bg-zinc-700" value="canceled" <?php echo (isset($_GET['statusFilter']) AND $_GET['statusFilter'] == "canceled") ? "selected" : ""; ?>> @lang('messages.t_canceled') </option>
                                                <option class="dark:bg-zinc-700" alue="refunded" <?php echo (isset($_GET['statusFilter']) AND $_GET['statusFilter'] == "refunded") ? "selected" : ""; ?>> @lang('messages.t_refunded')</option>
                                            </select>
                                        </div>
                                    </div>
            ...
            <table class="w-full text-left border-spacing-y-[10px] border-separate sm:mt-2" id="tabela">
                <thead class="">
                    <tr class="bg-slate-200 dark:bg-zinc-600">
                        ...
                        {{-- Gig --}}
                        <th sortable class="font-bold tracking-wider text-gray-600 px-5 py-4.5 border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md rtl:text-right">@lang('messages.t_gig')</th>
                        ...
                        {{-- Profit --}}
                        <th sortable class="font-bold tracking-wider text-gray-600 px-5 py-4.5 text-center border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md">@lang('messages.t_profit')</th>

                        {{-- Expected delivery date --}}
                        <th sortable class="font-bold tracking-wider text-gray-600 px-5 py-4.5 text-center border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md">@lang('messages.t_expected_delivery_date')</th>

                        {{-- Options --}}
                        <th sortable class="font-bold tracking-wider text-gray-600 px-5 py-4.5 text-center border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md">@lang('messages.t_options')</th>
                        ...
                        <tr class="intro-x shadow-sm bg-white dark:bg-zinc-800 rounded-md h-16" wire:key="freelancer-dashboard-orders-{{ $order->uid }}">
                            ...
                            <td class="px-5 py-3 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md text-center">
                                ...
                                @if ($order->refund && $order->refund->status === 'pending')
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-sm text-xs font-medium bg-red-50 text-red-800 dark:text-red-400 dark:bg-transparent">
                                        {{ __('messages.t_redund_requested') }}
                                    </span>
                                @elseif ($order->refund && $order->refund->status === 'rejected_by_admin')
                                <span class="inline-flex items-center px-2.5 py-1 rounded-sm text-xs font-medium bg-red-50 text-red-800 dark:text-red-400 dark:bg-transparent">
                                ...
                                @else
                                    @switch($order->status)
                                        ...
                                        {{-- Delivered --}}
                                        @case('waiting')
                                            @if ($order->refund && $order->refund->status === 'rejected_by_seller' && $order->refund->request_admin_intervention)
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-sm text-xs font-medium bg-red-50 text-red-800 dark:text-red-400 dark:bg-transparent">
                                            {{ __('messages.t_dispute_opened') }}
                                            </span><br>


                                            @elseif ($order->refund && $order->refund->status === 'rejected_by_seller')
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-sm text-xs font-medium bg-red-50 text-red-800 dark:text-red-400 dark:bg-transparent">
                                            {{ __('messages.t_u_have_declined_this_refund') }}
                                            </span><br>
                            ...
                            <td class="px-5 py-3 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md text-center">
                                @if ($order->expected_delivery_date)
                                    ...
                                    <?php echo date("d/m/Y \à\s h:i:s A", strtotime($order->expected_delivery_date)); ?> <!--{{ format_date($order->expected_delivery_date, config('carbon-formats.F_j,_Y_h_:_i_A')) }}-->
                                    </span>
                                @elseif (in_array($order->status, ['pending', 'proceeded']) && !$order->is_requirements_sent)
                                    ...
                                    -->
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-400">
                                        <span id="msg_agir_ate<?php echo $order->id;?>">{{ __('messages.t_agir_ate') }}:</span> <span id="TEMPORESTANTE<?php echo $order->id;?>"></span>
                                    </span>
                                @else
                            ...
                            <td class="px-5 py-3 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md text-center">
                                <div class="flex justify-center items-center space-x-2 rtl:space-x-reverse" id="hide_<?php echo $order->id; ?>">
                                    ...
                                    if(!$order->refund && $order->status === 'pending' && $order->order?->invoice && $order->order->invoice->status !== 'pending'){
                                        ...
                                        <a href="?id" wire:click="confirmProgress('{{ $order->uid }}')" 
                                        ...
                                        <!-- CANCELAR BTN -->
                                        <button wire:click="confirmCancel('{{ $order->uid }}')" type="button" data-tooltip-target="tooltip-actions-cancel-order-{{ $order->uid }}" 
                                        ...
                                        hover:text-white hover:bg-red-400 hover:border-red-400 hover:shadow focus:ring focus:ring-red-400 focus:ring-opacity-25 active:bg-red-500 active:border-red-500 active:shadow-none" wire:key="tooltip-actions-cancel-order-{{ $order->uid }}">
                                    ...
                                    <button wire:click="confirmCancel('{{ $order->uid }}')" type="button" data-tooltip-target="tooltip-actions-get-started-{{ $order->uid }}" class="inline-flex justify-center items-center border font-semibold focus:outline-none w-8 h-8 leading-5 text-sm rounded border-primary-300 bg-primary-100/20 text-primary-700 shadow-sm hover:text-white hover:bg-primary-400 hover:border-primary-400 hover:shadow focus:ring focus:ring-primary-400 focus:ring-opacity-25 active:bg-primary-500 active:border-primary-500 active:shadow-none" wire:key="tooltip-actions-get-started-{{ $order->uid }}">
                                        <svg class="w-4 h-4" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M20.92 2.38A15.72 15.72 0 0 0 17.5 2a8.26 8.26 0 0 0-6 2.06Q9.89 5.67 8.31 7.27c-1.21-.13-4.08-.2-6 1.74a1 1 0 0 0 0 1.41l11.3 11.32a1 1 0 0 0 1.41 0c1.95-2 1.89-4.82 1.77-6l3.21-3.2c3.19-3.19 1.74-9.18 1.68-9.43a1 1 0 0 0-.76-.73zm-2.36 8.75L15 14.67a1 1 0 0 0-.27.9 6.81 6.81 0 0 1-.54 3.94L4.52 9.82a6.67 6.67 0 0 1 4-.5A1 1 0 0 0 9.39 9s1.4-1.45 3.51-3.56A6.61 6.61 0 0 1 17.5 4a14.51 14.51 0 0 1 2.33.2c.24 1.43.62 5.04-1.27 6.93z"></path><circle cx="15.73" cy="8.3" r="2"></circle><path d="M5 16c-2 1-2 5-2 5a7.81 7.81 0 0 0 5-2z"></path></svg>
                                    </button>
                                    <x-forms.tooltip id="tooltip-actions-get-started-{{ $order->uid }}" :text="__('messages.t_get_started')" />
                                    -->
