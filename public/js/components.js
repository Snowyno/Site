...
window.Components = {}, window.Components.listbox = function(e) {
        ...
        init() {
            ...
            let e = i => {
                ...
                if (this.open && i.target instanceof Element && !n.contains(i.target)) {
                    let s = this.$el;
                    for (; s.parentNode;)
                        if ((s = s.parentNode).__x instanceof this.constructor) {
                            if ("popoverGroup" === s.__x.$data.__type) return;
                            if ("popover" === s.__x.$data.__type) break
                        } this.open = !1
                }
            };
            window.addEventListener("focus", e, !0)
    ...
    return {
        ...
        init() {
            ...
            for (let i of (this.value = t[e]?.value, t)) i.addEventListener("change", () => {
                this.active = i.value
