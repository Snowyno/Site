<?php 
//POR ALTASIS EM 09/05/2023
//Validando se passou 3 dias da order. Se sim, cancelar.

include_once('public/altasis/cancelOrderJob.php'); 
?>
                                        ...
                                        <tr wire:key="seller-orders-id-{{ $order->uid }}">
                                                ...
                                                <div class="flex items-center">
                                                    ...
                                                    <div class="ltr:ml-4 rtl:mr-4">
                                                        ...
                                                        if ($order->status == 'proceeded'){
                                                            //echo  '<span class="text-xs font-normal">TIMELEFT: 00:00:00 </span><br><Br>';
                                                        }
                                                        ?>
                                            ...
                                            <td class="text-center">
                                                @if ($order->expected_delivery_date)
                                                    <span class="text-xs font-medium text-gray-700 dark:text-gray-400"><?php echo date("d/m/Y à\s H:i:s", strtotime( $order->expected_delivery_date )); ?></span>
                                            ...
                                            <td class="text-center">
                                                <div class="relative inline-block text-left">
                                                    ...
                                                    <div id="seller-orders-options-{{ $order->uid }}" class="hidden z-40 origin-top-right absolute right-0 mt-2 w-44 rounded-md shadow-lg bg-white dark:bg-zinc-800 ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 dark:divide-zinc-700 focus:outline-none" role="menu"  aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                                        ...
                                                        {{-- Cancel --}}
                                                 @if ($order->status === 'pending' && $order->order->invoice && $order->order->invoice->status !== 'pending' || $order->status === 'proceeded' && $order->order->invoice && $order->order->invoice->status !== 'pending')
                                                            <div class="py-1" role="none">
