                            ...
                            @forelse ($orders as $order)
                                ...
                                <div class="bg-white dark:bg-zinc-700 border border-gray-200 dark:border-zinc-600 rounded-md mb-10" wire:key="orders-{{ $order->uid }}">
                                    <div class="bg-gray-50 dark:bg-zinc-600 border-b sm:rounded-t-md flex items-center p-4 sm:p-6 sm:grid sm:grid-cols-4 sm:gap-x-6 dark:border-zinc-600">
                                        ...
                                        {{-- Order quick stats --}}
                                        <dl class="flex-1 grid grid-cols-3 gap-x-6 text-sm sm:col-span-3 sm:grid-cols-3 lg:col-span-3">

                                            {{-- Date placed --}}
                                            <div>
                                                <dt class="font-medium text-gray-900 dark:text-gray-200">{{ __('messages.t_date_placed') }}</dt>
                                                <dd class="mt-2 text-gray-500 dark:text-gray-50 text-xs"><?php echo date("d/m/Y à\s H:i:s", strtotime( $order->placed_at )); ?></dd>
                                            </div>
                                            ...
                                            {{-- Total amount --}}
                                            <div>
                                                <dt class="font-medium text-gray-900 dark:text-gray-200">TIME LEFT:</dt>
                                                <dd class="mt-1 font-medium text-gray-900 dark:text-white"><span id="tempoQueFalta<?php echo $order->id; ?>"></span></dd>

                                                <script>
                                                
                                                    $( document ).ready(function() {
                                                        var intervalId = window.setInterval(function(){
                                                            timeLeft<?php echo $order->id; ?>();
                                                        }, 5000);

                                                    });


                                                    function timeLeft<?php echo $order->id; ?>(){
                                                        //alert('$order->placed_at');
                                                
                                                        $.post( "https://p2win.com.br/public/altasis/timeLeft.php", { placed_at: "<?php echo $order->placed_at; ?>",  id_order: "<?php echo $order->id; ?>"})
                                                        .done(function( data ) {
                                                            $("#tempoQueFalta<?php echo $order->id; ?>").html();
                                                            $("#tempoQueFalta<?php echo $order->id; ?>").html(data);
                                                                        ...
                                                                        <div class="md:text-center">
                                                                            ...
                                                                            @else
                                                                                @switch($item->status)
                                                                                    ...
                                                                                    @case('delivered')
                                                                                    @if($item->refund && $item->refund->status === 'rejected_by_seller')
                                                                                        <p data-tooltip-target="orders-{{ $order->uid }}-item-{{ $item->id }}-status-rejected-by-seller" class="mt-2 text-[11px] text-red-500 font-medium cursor-pointer">
                                                                                            {{ __('messages.t_declined_by_seller') }}
                                                                                        </p>
                                                                                        <div id="orders-{{ $order->uid }}-item-{{ $item->id }}-status-rejected-by-seller" role="tooltip" class="inline-block absolute invisible z-10 py-2 px-3 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                                                                            {{ format_date($item->declined_by_seller_at, 'F j, Y h:i A') }}
                                                                                        </div>
                                                                                    @elseif ($item->status === 'delivered' && $item->is_finished)
                                                                                        @if ($item->refund && $item->refund->status === 'rejected_by_admin')
                                                                                    <p data-tooltip-target="orders-{{ $order->uid }}-item-{{ $item->id }}-status-delivered" class="mt-2 text-[11px] text-red-500 font-medium cursor-pointer">
                                                                                            Reembolso Negado!
                                                                                        </p>
                                                                                    ...
                                                                                    {{-- Refunded --}}
                                                                                    @case('refunded')
                                                                                    @if($item->refund->status === 'rejected_by_admin')
                                                                                        <p data-tooltip-target="orders-{{ $order->uid }}-item-{{ $item->id }}-status-rejected-by-admin" class="mt-2 text-[11px] text-green-500 font-medium cursor-pointer">
                                                                                            {{ __('messages.t_declined_by_admin', ['app_name' => config('app.name')]) }}
                                                                                        </p>
                                                                                        <div id="orders-{{ $order->uid }}-item-{{ $item->id }}-status-rejected-by-seller" role="tooltip" class="inline-block absolute invisible z-10 py-2 px-3 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                                                                            {{ format_date($item->declined_by_admin_at, 'F j, Y h:i A') }}
                                                                                        </div>
                                                                                    @elseif($item->refund->status === 'accepted_by_seller')
                                                                                        <p data-tooltip-target="orders-{{ $order->uid }}-item-{{ $item->id }}-status-accepted-by-seller" class="mt-2 text-[11px] text-green-500 font-medium cursor-pointer">
                                                                                            {{ __('messages.t_approved_by_seller') }}
                                                                                        </p>
                                                                                        <div id="orders-{{ $order->uid }}-item-{{ $item->id }}-status-approved-by-seller" role="tooltip" class="inline-block absolute invisible z-10 py-2 px-3 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                                                                            {{ format_date($item->approved_by_seller_at, 'F j, Y h:i A') }}
                                                                                        </div>
                                                                                    @elseif ($item->refund->status === 'accepted_by_admin')
                                                                                        <p data-tooltip-target="orders-{{ $order->uid }}-item-{{ $item->id }}-status-accepted-by-admin" class="mt-2 text-[11px] text-green-500 font-medium cursor-pointer">
                                                                                            Reembolso Aprovado!
                                                                                        </p>
                                                                                        <div id="orders-{{ $order->uid }}-item-{{ $item->id }}-status-rejected-by-seller" role="tooltip" class="inline-block absolute invisible z-10 py-2 px-3 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                                                                            {{ format_date($item->approved_by_seller_at, 'F j, Y h:i A') }}
                                                            ...
                                                            <div class="mt-4 flex space-x-3 md:mt-0">
                                                                ...
                                                                <div id="orders-{{ $order->uid }}-item-{{ $item->id }}-actions-dropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded shadow w-48 dark:bg-zinc-800">
                                                                    <ul class="text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefault">
                                                                        ...
                                                                        {{-- View refund --}}
                                                                        @if ($item->refund && !$item->is_finished)
                                                                            <li>
