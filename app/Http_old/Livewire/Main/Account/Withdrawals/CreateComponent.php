...
namespace App\Http\Livewire\Main\Account\Withdrawals;
...
use App\Http\Validators\Main\Account\Withdrawals\MakeValidator;
use App\Notifications\Admin\PendingWithdrawal as AdminPendingWithdrawal;
use App\Notifications\User\Account\PendingWithdrawal as AccountPendingWithdrawal;
...
class CreateComponent extends Component
{
    ...
    public function mount()
    {
        ...
        if (!$user_withdrawal_settings) {
            return redirect('account/withdrawals/settings');
        }
        ...
        // Check customer default payout method
    ...
    public function render()
    {
        ...
        return view('livewire.main.account.withdrawals.create')->extends('livewire.main.account.layout.app')->section('content');
    }
    ...
    public function withdraw()
    {
        try {
            ...
            if ($pending) {
                ...
                // Cannot withdrawal right now
                return redirect('account/withdrawals')->with('message', __('messages.t_cant_withdraw_reason_pending_request'));
            ...
            if ($latest) {
                ...
                switch ($settings->withdrawal_period) {
                    ...
                    case 'daily':
                        ...
                        if ($latest->created_at->diffInHours($now) < 24) { // 24 hours = 1 day
                            return redirect('account/withdrawals')->with('message', __('messages.t_cant_withdraw_reason_period_24_hours'));
                        }
                    ...
                    case 'weekly':
                        ...
                        if ($latest->created_at->diffInHours($now) < 168) { // 168 hours = 7 days
                            return redirect('account/withdrawals')->with('message', __('messages.t_cant_withdraw_reason_period_7_days'));
                        }
                    ...
                    case 'monthly':
                        ...
                        if ($latest->created_at->diffInHours($now) < 720) { // 720 hours = 30 days
                            return redirect('account/withdrawals')->with('message', __('messages.t_cant_withdraw_reason_period_monthly'));
                        }
            ...
            auth()->user()->notify( (new AccountPendingWithdrawal($withdrawal))->locale(config('app.locale')) );
            
            // Success
            return redirect('account/withdrawals')->with('success', __('messages.t_ur_withdrawal_request_under_review'));
