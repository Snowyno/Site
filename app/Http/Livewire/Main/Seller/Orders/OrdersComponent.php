...
class OrdersComponent extends Component
{
    ...
    public function getOrdersProperty()
    {
        ...
        //$orders = OrderItem::where('owner_id', auth()->id())->first()->paginate(42);
        ...
        if(isset($_GET['statusFilter'])){
            ...
            ->where('status', '=', $_GET['statusFilter'])
            ->orderBy($_GET['col'], $_GET['order'])
            ->paginate(42);
    ...
    {
        // Get item
        $item = OrderItem::where('uid', $id)->where('owner_id', auth()->id())->firstOrFail();
        ...
        if ($item->order->invoice->status !== 'pending') {
            ...
            $item->order->buyer()->update([
                ...
                'balance_available' => convertToNumber($item->order->buyer->balance_available) + convertToNumber($item->profit_value)
            ]);
        }
