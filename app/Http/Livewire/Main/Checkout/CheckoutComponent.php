...
class CheckoutComponent extends Component
{
     ...
     * @return void
     */
    public function checkout($options = null, $aplicaCupon = null)
    {
        try {

            if($aplicaCupon !=null){
                $this->totalApagar = $aplicaCupon;
            }
            ...
            //VERIFICA SE NO MEIO TEMPO JÁ TEM PEDIDO
            // Get gig
            foreach($this->cart as $key => $item) {
                ...
                if($gig->orders_in_queue > 0){
                    //ITEM JA FOI COMPRADO, SAIR
                    $_SESSION['message'] = true;
                    return redirect('/search?esgotado');
            ...
            if (isset($response['success']) && $response['success'] === true) {
                ...
                //Update Cupon
                Cupon::where('id', $this->cuponId)
                ->update(['used_by' => $this->used_by]);
                ...
                if ($invoice->status === 'pending') {
                    ...
                    return redirect('account/orders')->with('message', __('messages.t_toast_operation_success'));
    ...
    protected function calculateExchangeAmount($gateway_exchange_rate = null)
    {
        try {
            ...
            // Get total amount
            if($this->totalApagar != null){
                $amount                = $this->totalApagar + $this->taxes();
            }
            else{
